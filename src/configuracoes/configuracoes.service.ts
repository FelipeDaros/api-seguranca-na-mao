...
export class ConfiguracoesService {
  ...
  public async create({ tipo, usuario_id, valor, parametro }: CreateConfiguracoeDto): Promise<Configuracoes> {
    try {
      const configExists = await this.prismaService.configuracoes.findFirst({
        where: {
          usuario_id,
          tipo
        }
      ...
      return await this.prismaService.configuracoes.create({
        data: {
          usuario_id,
          tipo,
          valor: Number(valor),
  ...
  public async update({ parametro, tipo, usuario_id, valor }: UpdateConfiguracoeDto): Promise<Configuracoes> {
    try {
      ...
      const configUpdate = await this.prismaService.configuracoes.update({
        where: {
          id: configExists.id
        },
        data: configExists
  ...
  public async buscarConfiguracoesVigilantesEmpresa(empresa_id: number): Promise<Usuario[]>{
    const vigilantes = await this.prismaService.usuario.findMany({
      where: {
        empresa_id,
  ...
  public async findOne(vigilante_id: string): Promise<Usuario>{
    const vigilante = await this.prismaService.usuario.findFirst({
      where: {
        id: vigilante_id,
        tipo_usuario: 'VIGILANTE'
      },
